...
export class PublicDownloadCountV2$ {
   ...
   */
  getDownloader_ByContentId_v2(contentId:string,  queryParams?: {limit?: number, offset?: number, sortBy?: string | null, userId?: string | null}): Promise<Response<PaginatedContentDownloaderResponse>> {
    const params = {limit: 20,sortBy: '*createdTime:desc*', ...queryParams} as AxiosRequestConfig
    const url = '/ugc/v2/public/namespaces/{namespace}/contents/{contentId}/downloader'.replace('{namespace}', this.namespace).replace('{contentId}', contentId)     
    const resultPromise = this.axiosInstance.get(url, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, PaginatedContentDownloaderResponse, 'PaginatedContentDownloaderResponse')
  }
  ...
  createDownloadcount_ByContentId_v2(contentId:string): Promise<Response<AddDownloadCountResponse>> {
    ...
    const url = '/ugc/v2/public/namespaces/{namespace}/contents/{contentId}/downloadcount'.replace('{namespace}', this.namespace).replace('{contentId}', contentId)     
