...
export class LeaderboardConfiguration$ {
   ...
   */
  getLeaderboards( queryParams?: {isArchived?: boolean | null, isDeleted?: boolean | null, limit?: number, offset?: number}): Promise<Response<GetAllLeaderboardConfigsPublicResp>> {
    const params = { ...queryParams} as AxiosRequestConfig
    const url = '/leaderboard/v1/public/namespaces/{namespace}/leaderboards'.replace('{namespace}', this.namespace)     
    const resultPromise = this.axiosInstance.get(url, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, GetAllLeaderboardConfigsPublicResp, 'GetAllLeaderboardConfigsPublicResp')
  }
