...
export const useUserDataAdminApi_GetLeaderboards_ByUserId = (
  ...
  const queryFn = (
  sdk: AccelByteSDK, 
  input: Parameters<typeof useUserDataAdminApi_GetLeaderboards_ByUserId>[1]
  ) => async () => {
      const response = 
            (await UserDataAdminApi(sdk, { coreConfig: input.coreConfig, axiosConfig: input.axiosConfig }).
                getLeaderboards_ByUserId(input.userId, input.queryParams))
