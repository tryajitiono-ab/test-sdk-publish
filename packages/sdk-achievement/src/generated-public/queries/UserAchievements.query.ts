...
export const useUserAchievementsApi_GetAchievements_ByUserId = (
  ...
  const queryFn = (sdk: AccelByteSDK, input: Parameters<typeof useUserAchievementsApi_GetAchievements_ByUserId>[1]) => async () => {
    const response = await UserAchievementsApi(sdk, {
 ...
 * ```
 * {
 *    queryKey: [Key_UserAchievements.Unlock_ByUserId_ByAchievementCode, input]
 * }
 * ```
 */
