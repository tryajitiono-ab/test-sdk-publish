...
export const useAchievementsApi_GetAchievements = (
    sdk: AccelByteSDK,
    input: SdkSetConfigParam & {  queryParams: {language: string | null, global?: boolean | null, limit?: number, offset?: number, sortBy?: 'listOrder' | 'listOrder:asc' | 'listOrder:desc' | 'createdAt' | 'createdAt:asc' | 'createdAt:desc' | 'updatedAt' | 'updatedAt:asc' | 'updatedAt:desc', tags?: string[]} },
  ...
  const queryFn = (
  sdk: AccelByteSDK, 
  input: Parameters<typeof useAchievementsApi_GetAchievements>[1]
  ) => async () => {
      const response = 
            (await AchievementsApi(sdk, { coreConfig: input.coreConfig, axiosConfig: input.axiosConfig }).
...
export const useAchievementsApi_GetAchievement_ByAchievementCode = (
  ...
  const queryFn = (
  sdk: AccelByteSDK, 
  input: Parameters<typeof useAchievementsApi_GetAchievement_ByAchievementCode>[1]
  ) => async () => {
      const response = 
            (await AchievementsApi(sdk, { coreConfig: input.coreConfig, axiosConfig: input.axiosConfig }).
