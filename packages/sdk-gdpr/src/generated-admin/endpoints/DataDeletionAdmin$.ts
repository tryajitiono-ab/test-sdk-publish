...
export class DataDeletionAdmin$ {
   ...
   */
  getDeletions( queryParams?: {after?: string | null, before?: string | null, limit?: number, offset?: number, requestDate?: string | null}): Promise<Response<ListDeletionDataResponse>> {
    const params = { ...queryParams} as AxiosRequestConfig
    const url = '/gdpr/admin/namespaces/{namespace}/deletions'.replace('{namespace}', this.namespace)     
    const resultPromise = this.axiosInstance.get(url, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, ListDeletionDataResponse, 'ListDeletionDataResponse')
  }
  ...
  deleteDeletion_ByUserId(userId:string): Promise<Response<unknown>> {
    ...
    const url = '/gdpr/admin/namespaces/{namespace}/users/{userId}/deletions'.replace('{namespace}', this.namespace).replace('{userId}', userId)     
