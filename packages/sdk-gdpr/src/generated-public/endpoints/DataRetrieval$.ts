...
export class DataRetrieval$ {
  ...
  getRequests_ByUserId(userId:string,  queryParams?: {limit?: number, offset?: number}): Promise<Response<UserPersonalDataResponse>> {
    ...
    const url = '/gdpr/public/namespaces/{namespace}/users/{userId}/requests'.replace('{namespace}', this.namespace).replace('{userId}', userId)     
   ...
   */
  postRequest_ByUserId(userId:string, data: {password: string | null,languageTag?: string | null}): Promise<Response<DataRetrievalResponse>> {
  ...
  deleteRequest_ByUserId_ByRequestDate(userId:string, requestDate:string): Promise<Response<unknown>> {
    const params = {} as AxiosRequestConfig
    const url = '/gdpr/public/namespaces/{namespace}/users/{userId}/requests/{requestDate}'.replace('{namespace}', this.namespace).replace('{userId}', userId).replace('{requestDate}', requestDate)     
  ...
  postGenerate_ByUserId_ByRequestDate(userId:string, requestDate:string, data: {password: string | null}): Promise<Response<UserDataUrl>> {
    const params = {} as AxiosRequestConfig
    const url = '/gdpr/public/namespaces/{namespace}/users/{userId}/requests/{requestDate}/generate'.replace('{namespace}', this.namespace).replace('{userId}', userId).replace('{requestDate}', requestDate)     
    const resultPromise = this.axiosInstance.post(url, CodeGenUtil.getFormUrlEncodedData(data), { ...params, headers: { ...params.headers, 'content-type': 'application/x-www-form-urlencoded' } })
