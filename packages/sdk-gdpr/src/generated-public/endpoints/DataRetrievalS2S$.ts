...
export class DataRetrievalS2S$ {
  ...
  getS2SRequestsFinished( queryParams: {end: string | null, start: string | null}): Promise<Response<ListFinishedDataRequests>> {
    ...
    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, ListFinishedDataRequests, 'ListFinishedDataRequests')
  }
  ...
  createRequestS2_ByUserId(userId:string): Promise<Response<S2SDataRetrievalResponse>> {
    ...
    const url = '/gdpr/s2s/namespaces/{namespace}/users/{userId}/requests'.replace('{namespace}', this.namespace).replace('{userId}', userId)     
    const resultPromise = this.axiosInstance.post(url, null, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, S2SDataRetrievalResponse, 'S2SDataRetrievalResponse')
  }
  ...
  fetchGenerateS2_ByUserId_ByRequestDate(userId:string, requestDate:string): Promise<Response<S2SUserDataUrl>> {
    const params = {} as AxiosRequestConfig
    const url = '/gdpr/s2s/namespaces/{namespace}/users/{userId}/requests/{requestDate}/generate'.replace('{namespace}', this.namespace).replace('{userId}', userId).replace('{requestDate}', requestDate)     
    const resultPromise = this.axiosInstance.post(url, null, {params})
