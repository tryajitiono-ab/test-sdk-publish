...
export class DataDeletionS2S$ {
  ...
  getS2SDeletionsFinished( queryParams: {end: string | null, start: string | null}): Promise<Response<ListFinishedDataDeletion>> {
    ...
    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, ListFinishedDataDeletion, 'ListFinishedDataDeletion')
  }
  ...
  createDeletionS2_ByUserId(userId:string): Promise<Response<S2SRequestDeleteResponse>> {
    ...
    const url = '/gdpr/s2s/namespaces/{namespace}/users/{userId}/deletions'.replace('{namespace}', this.namespace).replace('{userId}', userId)     
    const resultPromise = this.axiosInstance.post(url, null, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, S2SRequestDeleteResponse, 'S2SRequestDeleteResponse')
  }
