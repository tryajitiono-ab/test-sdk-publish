...
export class EventV2Admin$ {
   ...
   */
  fetchQuery_v2(data: GenericQueryPayload, queryParams?: {endDate?: string | null, offset?: number, pageSize?: number, startDate?: string | null}): Promise<Response<EventResponseV2>> {
  ...
  getEvent_ByUserId_v2(userId:string,  queryParams?: {endDate?: string | null, eventName?: string | null, offset?: number, pageSize?: number, startDate?: string | null}): Promise<Response<EventResponseV2>> {
    const params = { ...queryParams} as AxiosRequestConfig
    const url = '/event/v2/admin/namespaces/{namespace}/users/{userId}/event'.replace('{namespace}', this.namespace).replace('{userId}', userId)     
