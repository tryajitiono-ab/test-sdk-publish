...
export class ReleaseNoteAdmin$ {
  ...
  patchReleasenoteUploadCommit_ByHash(hash:string): Promise<Response<BlockManifest>> {
    ...
    const url = '/buildinfo/admin/namespaces/{namespace}/releasenote/upload/commit/{hash}'.replace('{namespace}', this.namespace).replace('{hash}', hash)     
  ...
  createReleasenoteUploadStart_ByUploaderId(uploaderId:string, data: BinaryUpload): Promise<Response<BlockManifest>> {
    const params = {} as AxiosRequestConfig
    const url = '/buildinfo/admin/namespaces/{namespace}/releasenote/upload/start/{uploaderId}'.replace('{namespace}', this.namespace).replace('{uploaderId}', uploaderId)     
  ...
  createReleasenoteManifestSave_ByUploadMode(uploadMode:string, data: ReleaseNoteManifest): Promise<Response<unknown>> {
    const params = {} as AxiosRequestConfig
    const url = '/buildinfo/admin/namespaces/{namespace}/releasenote/manifest/save/{uploadMode}'.replace('{namespace}', this.namespace).replace('{uploadMode}', uploadMode)     
    const resultPromise = this.axiosInstance.post(url, data, {params})
   ...
   */
  getReleasenoteManifestGet_ByAppId_ByPlatformId(appId:string, platformId:string,  queryParams?: {version?: string | null}): Promise<Response<ReleaseNoteDto>> {
    const params = { ...queryParams} as AxiosRequestConfig
    const url = '/buildinfo/admin/namespaces/{namespace}/releasenote/manifest/get/{appId}/{platformId}'.replace('{namespace}', this.namespace).replace('{appId}', appId).replace('{platformId}', platformId)     
