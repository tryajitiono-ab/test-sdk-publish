...
export class MultipartUploaderAdmin$ {
  ...
  createBlockMultipart(data: StartMultipartUploadRequest): Promise<Response<MultipartUploadSummary>> {
    ...
    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, MultipartUploadSummary, 'MultipartUploadSummary')
  }
  ...
  deleteBlockMultipart_ByHash(hash:string): Promise<Response<BlockManifest>> {
    const params = {} as AxiosRequestConfig
    const url = '/buildinfo/admin/namespaces/{namespace}/blocks/multipart/{hash}'.replace('{namespace}', this.namespace).replace('{hash}', hash)     
  ...
  deleteBlockMultipart_ByHash_v2(hash:string): Promise<Response<BlockManifest>> {
    const params = {} as AxiosRequestConfig
    const url = '/buildinfo/v2/admin/namespaces/{namespace}/blocks/multipart/{hash}'.replace('{namespace}', this.namespace).replace('{hash}', hash)     
    const resultPromise = this.axiosInstance.delete(url, {params})
  ...
  createPartBlock_ByHash_v2(hash:string, data: MultipartUploadedPart): Promise<Response<MultipartUploadSummary>> {
    const params = {} as AxiosRequestConfig
    const url = '/buildinfo/v2/admin/namespaces/{namespace}/blocks/multipart/{hash}/part'.replace('{namespace}', this.namespace).replace('{hash}', hash)     
    const resultPromise = this.axiosInstance.post(url, data, {params})
