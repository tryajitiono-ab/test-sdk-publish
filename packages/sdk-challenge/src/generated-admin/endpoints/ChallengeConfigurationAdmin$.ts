...
export class ChallengeConfigurationAdmin$ {
   ...
   */
  getChallenges( queryParams?: {limit?: number, offset?: number, sortBy?: string | null, status?: 'INIT' | 'RETIRED' | 'TIED'}): Promise<Response<ListChallengeResponse>> {
  ...
  deleteChallenge_ByChallengeCode(challengeCode:string): Promise<Response<unknown>> {
    const params = {} as AxiosRequestConfig
    const url = '/challenge/v1/admin/namespaces/{namespace}/challenges/{challengeCode}'.replace('{namespace}', this.namespace).replace('{challengeCode}', challengeCode)     
  ...
  deleteTied_ByChallengeCode(challengeCode:string): Promise<Response<unknown>> {
    const params = {} as AxiosRequestConfig
    const url = '/challenge/v1/admin/namespaces/{namespace}/challenges/{challengeCode}/tied'.replace('{namespace}', this.namespace).replace('{challengeCode}', challengeCode)     
    const resultPromise = this.axiosInstance.delete(url, {params})
   ...
   * &lt;ul&gt;&lt;li&gt;Required permission: ADMIN:NAMESPACE:{namespace}:CHALLENGE [READ]&lt;/li&gt;&lt;/ul&gt; 
   */
  getPeriods_ByChallengeCode(challengeCode:string,  queryParams?: {limit?: number, offset?: number}): Promise<Response<ListPeriodsResponse>> {
    const params = {limit: 20, ...queryParams} as AxiosRequestConfig
    const url = '/challenge/v1/admin/namespaces/{namespace}/challenges/{challengeCode}/periods'.replace('{namespace}', this.namespace).replace('{challengeCode}', challengeCode)     
    const resultPromise = this.axiosInstance.get(url, {params})
  ...
  createRandomize_ByChallengeCode(challengeCode:string): Promise<Response<ScheduleArray>> {
    const params = {} as AxiosRequestConfig
    const url = '/challenge/v1/admin/namespaces/{namespace}/challenges/{challengeCode}/randomize'.replace('{namespace}', this.namespace).replace('{challengeCode}', challengeCode)     
