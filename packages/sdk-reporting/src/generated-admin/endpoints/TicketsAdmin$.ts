...
export class TicketsAdmin$ {
   ...
   */
  getTickets( queryParams?: {category?: string | null, extensionCategory?: string | null, limit?: number, offset?: number, order?: string | null, reportedUserId?: string | null, sortBy?: string | null, status?: string | null}): Promise<Response<TicketListResponse>> {
   ...
   */
  getTicketsStatistic( queryParams: {category: string | null, extensionCategory?: string | null}): Promise<Response<TicketStatisticResponse>> {
    const params = { ...queryParams} as AxiosRequestConfig
    const url = '/reporting/v1/admin/namespaces/{namespace}/tickets/statistic'.replace('{namespace}', this.namespace)     
    const resultPromise = this.axiosInstance.get(url, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, TicketStatisticResponse, 'TicketStatisticResponse')
  }
  ...
  deleteTicket_ByTicketId(ticketId:string): Promise<Response<unknown>> {
    ...
    const url = '/reporting/v1/admin/namespaces/{namespace}/tickets/{ticketId}'.replace('{namespace}', this.namespace).replace('{ticketId}', ticketId)     
  ...
  getReports_ByTicketId(ticketId:string,  queryParams?: {limit?: number, offset?: number}): Promise<Response<ReportListResponse>> {
    const params = { ...queryParams} as AxiosRequestConfig
    const url = '/reporting/v1/admin/namespaces/{namespace}/tickets/{ticketId}/reports'.replace('{namespace}', this.namespace).replace('{ticketId}', ticketId)     
    const resultPromise = this.axiosInstance.get(url, {params})
  ...
  updateResolution_ByTicketId(ticketId:string, data: UpdateTicketResolutionsRequest): Promise<Response<TicketResponse>> {
    const params = {} as AxiosRequestConfig
    const url = '/reporting/v1/admin/namespaces/{namespace}/tickets/{ticketId}/resolutions'.replace('{namespace}', this.namespace).replace('{ticketId}', ticketId)     
