...
export class PublicReasons$ {
  ...
  getReasons( queryParams?: {group?: string | null, limit?: number, offset?: number, title?: string | null}): Promise<Response<PublicReasonListResponse>> {
    const params = { ...queryParams} as AxiosRequestConfig
    const url = '/reporting/v1/public/namespaces/{namespace}/reasons'.replace('{namespace}', this.namespace)     
    const resultPromise = this.axiosInstance.get(url, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, PublicReasonListResponse, 'PublicReasonListResponse')
  }
