...
export class GlobalStatistic$ {
   ...
   */
  getGlobalstatitems( queryParams?: {limit?: number, offset?: number, statCodes?: string | null}): Promise<Response<GlobalStatItemPagingSlicedResult>> {
    const params = {limit: 20, ...queryParams} as AxiosRequestConfig
    const url = '/social/v1/public/namespaces/{namespace}/globalstatitems'.replace('{namespace}', this.namespace)     
    const resultPromise = this.axiosInstance.get(url, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, GlobalStatItemPagingSlicedResult, 'GlobalStatItemPagingSlicedResult')
  }
  ...
  getGlobalstatitem_ByStatCode(statCode:string): Promise<Response<GlobalStatItemInfo>> {
    ...
    const url = '/social/v1/public/namespaces/{namespace}/globalstatitems/{statCode}'.replace('{namespace}', this.namespace).replace('{statCode}', statCode)     
    const resultPromise = this.axiosInstance.get(url, {params})
