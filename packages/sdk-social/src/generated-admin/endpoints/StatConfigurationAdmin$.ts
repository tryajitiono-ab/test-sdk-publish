...
export class StatConfigurationAdmin$ {
   ...
   */
  getStats( queryParams?: {cycleIds?: string | null, isGlobal?: boolean | null, isPublic?: boolean | null, limit?: number, offset?: number}): Promise<Response<StatPagingSlicedResult>> {
    const params = {limit: 20, ...queryParams} as AxiosRequestConfig
    const url = '/social/v1/admin/namespaces/{namespace}/stats'.replace('{namespace}', this.namespace)     
    const resultPromise = this.axiosInstance.get(url, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, StatPagingSlicedResult, 'StatPagingSlicedResult')
  }
...
// TODO file upload not implemented
   ...
   */
  getStatsSearch( queryParams: {keyword: string | null, isGlobal?: boolean | null, isPublic?: boolean | null, limit?: number, offset?: number}): Promise<Response<StatPagingSlicedResult>> {
    const params = {limit: 20, ...queryParams} as AxiosRequestConfig
  ...
  deleteStat_ByStatCode(statCode:string): Promise<Response<unknown>> {
    const params = {} as AxiosRequestConfig
    const url = '/social/v1/admin/namespaces/{namespace}/stats/{statCode}'.replace('{namespace}', this.namespace).replace('{statCode}', statCode)     
  ...
  deleteTied_ByStatCode(statCode:string): Promise<Response<unknown>> {
    const params = {} as AxiosRequestConfig
    const url = '/social/v1/admin/namespaces/{namespace}/stats/{statCode}/tied'.replace('{namespace}', this.namespace).replace('{statCode}', statCode)     
    const resultPromise = this.axiosInstance.delete(url, {params})
