...
export class SlotConfigAdmin$ {
  ...
  getConfig(): Promise<Response<NamespaceSlotConfigInfo>> {
    ...
    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, NamespaceSlotConfigInfo, 'NamespaceSlotConfigInfo')
  }
  ...
  deleteConfig_ByUserId(userId:string): Promise<Response<unknown>> {
    const params = {} as AxiosRequestConfig
    const url = '/social/admin/namespaces/{namespace}/users/{userId}/config'.replace('{namespace}', this.namespace).replace('{userId}', userId)     
    const resultPromise = this.axiosInstance.delete(url, {params})
