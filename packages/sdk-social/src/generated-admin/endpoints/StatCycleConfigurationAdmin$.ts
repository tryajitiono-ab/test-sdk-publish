...
export class StatCycleConfigurationAdmin$ {
   ...
   */
  getStatCycles( queryParams?: {cycleType?: 'ANNUALLY' | 'DAILY' | 'MONTHLY' | 'SEASONAL' | 'WEEKLY', limit?: number, name?: string | null, offset?: number, sortBy?: string | null, status?: 'ACTIVE' | 'INIT' | 'STOPPED'}): Promise<Response<StatCyclePagingSlicedResult>> {
    const params = {limit: 20, ...queryParams} as AxiosRequestConfig
    const url = '/social/v1/admin/namespaces/{namespace}/statCycles'.replace('{namespace}', this.namespace)     
    const resultPromise = this.axiosInstance.get(url, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, StatCyclePagingSlicedResult, 'StatCyclePagingSlicedResult')
  }
...
// TODO file upload not implemented
  ...
  deleteStatCycle_ByCycleId(cycleId:string): Promise<Response<unknown>> {
    const params = {} as AxiosRequestConfig
    const url = '/social/v1/admin/namespaces/{namespace}/statCycles/{cycleId}'.replace('{namespace}', this.namespace).replace('{cycleId}', cycleId)     
  ...
  updateStop_ByCycleId(cycleId:string): Promise<Response<StatCycleInfo>> {
    const params = {} as AxiosRequestConfig
    const url = '/social/v1/admin/namespaces/{namespace}/statCycles/{cycleId}/stop'.replace('{namespace}', this.namespace).replace('{cycleId}', cycleId)     
  ...
  createStatAddBulk_ByCycleId(cycleId:string, data: BulkCycleStatsAdd): Promise<Response<BulkStatCycleOperationResultArray>> {
    const params = {} as AxiosRequestConfig
    const url = '/social/v1/admin/namespaces/{namespace}/statCycles/{cycleId}/stats/add/bulk'.replace('{namespace}', this.namespace).replace('{cycleId}', cycleId)     
    const resultPromise = this.axiosInstance.post(url, data, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, BulkStatCycleOperationResultArray, 'BulkStatCycleOperationResultArray')
  }
