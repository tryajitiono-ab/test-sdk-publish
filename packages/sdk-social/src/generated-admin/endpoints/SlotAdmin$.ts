...
export class SlotAdmin$ {
  ...
  getSlots_ByUserId(userId:string): Promise<Response<SlotInfoArray>> {
    ...
    const url = '/social/admin/namespaces/{namespace}/users/{userId}/slots'.replace('{namespace}', this.namespace).replace('{userId}', userId)     
  ...
  getSlot_ByUserId_BySlotId(userId:string, slotId:string): Promise<Response<unknown>> {
    const params = {} as AxiosRequestConfig
    const url = '/social/admin/namespaces/{namespace}/users/{userId}/slots/{slotId}'.replace('{namespace}', this.namespace).replace('{userId}', userId).replace('{slotId}', slotId)     
    const resultPromise = this.axiosInstance.get(url, {params})
