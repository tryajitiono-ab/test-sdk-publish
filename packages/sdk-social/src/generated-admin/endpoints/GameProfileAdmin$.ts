...
export class GameProfileAdmin$ {
  ...
  getProfiles_ByUserId(userId:string): Promise<Response<GameProfileHeaderArray>> {
    ...
    const url = '/social/admin/namespaces/{namespace}/users/{userId}/profiles'.replace('{namespace}', this.namespace).replace('{userId}', userId)     
  ...
  getProfile_ByUserId_ByProfileId(userId:string, profileId:string): Promise<Response<GameProfileInfo>> {
    const params = {} as AxiosRequestConfig
    const url = '/social/admin/namespaces/{namespace}/users/{userId}/profiles/{profileId}'.replace('{namespace}', this.namespace).replace('{userId}', userId).replace('{profileId}', profileId)     
    const resultPromise = this.axiosInstance.get(url, {params})
