...
export class GameRecordAdmin$ {
   ...
   */
  getRecords( queryParams: {limit: number, offset: number, query?: string | null, tags?: string[]}): Promise<Response<ListGameRecordKeysResponse>> {
    const params = { ...queryParams} as AxiosRequestConfig
    const url = '/cloudsave/v1/admin/namespaces/{namespace}/records'.replace('{namespace}', this.namespace)     
    const resultPromise = this.axiosInstance.get(url, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, ListGameRecordKeysResponse, 'ListGameRecordKeysResponse')
  }
  ...
  getRecord_ByKey(key:string): Promise<Response<GameRecordAdminResponse>> {
    ...
    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, GameRecordAdminResponse, 'GameRecordAdminResponse')
  }
