...
export class EmailSenderTemplateAdmin$ {
  ...
  getEmailtemplatesEmailsender_ByAccount(account:string): Promise<Response<EmailTemplatePairArray>> {
    ...
    const url = '/config/v1/admin/namespaces/{namespace}/emailsender/accounts/{account}/emailtemplates'.replace('{namespace}', this.namespace).replace('{account}', account)     
    const resultPromise = this.axiosInstance.get(url, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, EmailTemplatePairArray, 'EmailTemplatePairArray')
  }
