...
export class ConfigurationTemplateAdmin$ {
  ...
  createConfiguration(data: CreateConfigurationTemplateRequest): Promise<Response<ConfigurationTemplateResponse>> {
    ...
    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, ConfigurationTemplateResponse, 'ConfigurationTemplateResponse')
  }
  
  /**
   * Get all template configurations in specified namespace. 
   */
  getConfigurations( queryParams?: {limit?: number, name?: string | null, offset?: number, order?: string | null, orderBy?: string | null}): Promise<Response<ConfigurationTemplatesResponse>> {
    const params = {limit: 20, ...queryParams} as AxiosRequestConfig
    const url = '/session/v1/admin/namespaces/{namespace}/configurations'.replace('{namespace}', this.namespace)     
    const resultPromise = this.axiosInstance.get(url, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, ConfigurationTemplatesResponse, 'ConfigurationTemplatesResponse')
  }
