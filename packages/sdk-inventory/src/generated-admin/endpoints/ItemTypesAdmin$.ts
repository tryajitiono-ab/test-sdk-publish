...
export class ItemTypesAdmin$ {
   ...
   */
  getItemtypes( queryParams?: {limit?: number, offset?: number, sortBy?: 'createdAt' | 'createdAt:asc' | 'createdAt:desc' | 'name' | 'name:asc' | 'name:desc'}): Promise<Response<ListItemTypesResp>> {
  ...
  deleteItemtype_ByItemTypeName(itemTypeName:string): Promise<Response<unknown>> {
    const params = {} as AxiosRequestConfig
    const url = '/inventory/v1/admin/namespaces/{namespace}/itemtypes/{itemTypeName}'.replace('{namespace}', this.namespace).replace('{itemTypeName}', itemTypeName)     
