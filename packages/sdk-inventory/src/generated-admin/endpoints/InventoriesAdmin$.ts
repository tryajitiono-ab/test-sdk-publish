...
export class InventoriesAdmin$ {
   ...
   */
  getInventories( queryParams?: {inventoryConfigurationCode?: string | null, limit?: number, offset?: number, sortBy?: 'createdAt' | 'createdAt:asc' | 'createdAt:desc' | 'inventoryConfigurationCode' | 'inventoryConfigurationCode:asc' | 'inventoryConfigurationCode:desc' | 'updatedAt' | 'updatedAt:asc' | 'updatedAt:desc', userId?: string | null}): Promise<Response<ListInventoryResp>> {
  ...
  deleteInventory_ByInventoryId(inventoryId:string, data: DeleteInventoryReq): Promise<Response<unknown>> {
    const params = {} as AxiosRequestConfig
    const url = '/inventory/v1/admin/namespaces/{namespace}/inventories/{inventoryId}'.replace('{namespace}', this.namespace).replace('{inventoryId}', inventoryId)     
   ...
   */
  createPurchaseable_ByUserId(userId:string, data: PurchaseValidationReq): Promise<Response<unknown>> {
    const params = {} as AxiosRequestConfig
    const url = '/inventory/v1/admin/namespaces/{namespace}/users/{userId}/purchaseable'.replace('{namespace}', this.namespace).replace('{userId}', userId)     
    const resultPromise = this.axiosInstance.post(url, data, {params})
   ...
   */
  updateInventory_ByUserId_ByInventoryConfigurationCode(userId:string, inventoryConfigurationCode:string, data: UpdateInventoryReq): Promise<Response<InventoryRespArray>> {
    const params = {} as AxiosRequestConfig
    const url = '/inventory/v1/admin/namespaces/{namespace}/users/{userId}/inventoryConfigurations/{inventoryConfigurationCode}/inventories'.replace('{namespace}', this.namespace).replace('{userId}', userId).replace('{inventoryConfigurationCode}', inventoryConfigurationCode)     
    const resultPromise = this.axiosInstance.put(url, data, {params})
