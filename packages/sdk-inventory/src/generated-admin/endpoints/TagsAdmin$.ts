...
export class TagsAdmin$ {
   ...
   */
  getTags( queryParams?: {limit?: number, offset?: number, owner?: string | null, sortBy?: 'createdAt' | 'createdAt:asc' | 'createdAt:desc' | 'name' | 'name:asc' | 'name:desc'}): Promise<Response<ListTagsResp>> {
  ...
  deleteTag_ByTagName(tagName:string): Promise<Response<unknown>> {
    const params = {} as AxiosRequestConfig
    const url = '/inventory/v1/admin/namespaces/{namespace}/tags/{tagName}'.replace('{namespace}', this.namespace).replace('{tagName}', tagName)     
