...
export function OAuth20V4Api(sdk: AccelByteSDK, args?: SdkSetConfigParam) {
  ...
  async function postOauthToken_v4(
    data: {
      grant_type:
        | 'authorization_code'
        | 'client_credentials'
  ...
  async function postOauthTokenExchange_v4(data: { code: string | null; additionalData?: string | null }): Promise<AxiosResponse<unknown>> {
    const $ = new OAuth20V4$(axiosInstance, namespace, useSchemaValidation)
  ...
  async function postOauthSimultaneousLogin_v4(
    data: {
  ...
  async function postTokenOauth_ByPlatformId_v4(
    platformId: string,
    data: {
      additionalData?: string | null
      client_id?: string | null
