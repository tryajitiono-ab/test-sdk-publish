...
export class GroupAdmin$ {
   ...
   */
  getGroups( queryParams?: {configurationCode?: string | null, groupName?: string | null, groupRegion?: string | null, limit?: number, offset?: number}): Promise<Response<GetGroupsListResponseV1>> {
  ...
  deleteGroup_ByGroupId(groupId:string): Promise<Response<unknown>> {
    const params = {} as AxiosRequestConfig
    const url = '/group/v1/admin/namespaces/{namespace}/groups/{groupId}'.replace('{namespace}', this.namespace).replace('{groupId}', groupId)     
