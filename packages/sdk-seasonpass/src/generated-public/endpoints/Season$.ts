...
export class Season$ {
   ...
   */
  getSeasonsCurrent( queryParams?: {language?: string | null}): Promise<Response<LocalizedSeasonInfo>> {
   ...
   */
  getSeasonsCurrentData_ByUserId(userId:string): Promise<Response<ClaimableUserSeasonInfo>> {
    const params = {} as AxiosRequestConfig
    const url = '/seasonpass/public/namespaces/{namespace}/users/{userId}/seasons/current/data'.replace('{namespace}', this.namespace).replace('{userId}', userId)     
    const resultPromise = this.axiosInstance.get(url, {params})
  ...
  getData_ByUserId_BySeasonId(userId:string, seasonId:string): Promise<Response<ClaimableUserSeasonInfo>> {
    const params = {} as AxiosRequestConfig
    const url = '/seasonpass/public/namespaces/{namespace}/users/{userId}/seasons/{seasonId}/data'.replace('{namespace}', this.namespace).replace('{userId}', userId).replace('{seasonId}', seasonId)     
    const resultPromise = this.axiosInstance.get(url, {params})
