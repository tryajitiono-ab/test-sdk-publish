...
export function PaymentAdminApi(sdk: AccelByteSDK, args?: SdkSetConfigParam) {
   ...
   */
  async function getPaymentOrders( queryParams?: {channel?: 'EXTERNAL' | 'INTERNAL', extTxId?: string | null, limit?: number, offset?: number, status?: 'AUTHORISED' | 'AUTHORISE_FAILED' | 'CHARGEBACK' | 'CHARGEBACK_REVERSED' | 'CHARGED' | 'CHARGE_FAILED' | 'DELETED' | 'INIT' | 'NOTIFICATION_OF_CHARGEBACK' | 'REFUNDED' | 'REFUNDING' | 'REFUND_FAILED' | 'REQUEST_FOR_INFORMATION'}): Promise<AxiosResponse<PaymentOrderPagingSlicedResult>> {
   ...
   */
  async function getPaymentNotifications( queryParams?: {endDate?: string | null, externalId?: string | null, limit?: number, notificationSource?: 'ADYEN' | 'ALIPAY' | 'CHECKOUT' | 'NEONPAY' | 'PAYPAL' | 'STRIPE' | 'WALLET' | 'WXPAY' | 'XSOLLA', notificationType?: string | null, offset?: number, paymentOrderNo?: string | null, startDate?: string | null, status?: 'ERROR' | 'IGNORED' | 'PROCESSED' | 'WARN'}): Promise<AxiosResponse<PaymentNotificationPagingSlicedResult>> {
    const $ = new PaymentAdmin$(axiosInstance, namespace, useSchemaValidation)
   ...
   */
  async function updatePaymentOrder_ByPaymentOrderNo(paymentOrderNo:string, data: PaymentOrderChargeRequest): Promise<AxiosResponse<PaymentOrderInfo>> {
    const $ = new PaymentAdmin$(axiosInstance, namespace, useSchemaValidation)
   ...
   */
  async function updateSimulateNotificationPayment_ByPaymentOrderNo(paymentOrderNo:string, data: PaymentOrderNotifySimulation): Promise<AxiosResponse<NotificationProcessResult>> {
    const $ = new PaymentAdmin$(axiosInstance, namespace, useSchemaValidation)
   ...
   */
  async function updateRefundPayment_ByUserId_ByPaymentOrderNo(userId:string, paymentOrderNo:string, data: PaymentOrderRefund): Promise<AxiosResponse<PaymentOrderInfo>> {
    const $ = new PaymentAdmin$(axiosInstance, namespace, useSchemaValidation)
  ...
  return {
    getPaymentOrders,getPaymentNotifications,getPaymentOrdersByExtTxId,createPaymentOrder_ByUserId,getPaymentOrder_ByPaymentOrderNo,updatePaymentOrder_ByPaymentOrderNo,getStatusPayment_ByPaymentOrderNo,updateSimulateNotificationPayment_ByPaymentOrderNo,updateRefundPayment_ByUserId_ByPaymentOrderNo,
  }
}
