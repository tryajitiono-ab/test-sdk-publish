...
export class SectionAdmin$ {
   ...
   */
  getSections( queryParams?: {end?: string | null, limit?: number, offset?: number, start?: string | null, storeId?: string | null, viewId?: string | null}): Promise<Response<SectionPagingSlicedResult>> {
    const params = {limit: 20, ...queryParams} as AxiosRequestConfig
    const url = '/platform/admin/namespaces/{namespace}/sections'.replace('{namespace}', this.namespace)     
    const resultPromise = this.axiosInstance.get(url, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, SectionPagingSlicedResult, 'SectionPagingSlicedResult')
  }
   ...
   */
  updateSection_BySectionId(sectionId:string, data: SectionUpdate, queryParams: {storeId: string | null}): Promise<Response<FullSectionInfo>> {
    const params = { ...queryParams} as AxiosRequestConfig
    const url = '/platform/admin/namespaces/{namespace}/sections/{sectionId}'.replace('{namespace}', this.namespace).replace('{sectionId}', sectionId)     
