...
export class RevocationAdmin$ {
   ...
   */
  getRevocationHistory( queryParams?: {endTime?: string | null, limit?: number, offset?: number, source?: 'DLC' | 'IAP' | 'ORDER' | 'OTHER', startTime?: string | null, status?: 'FAIL' | 'SUCCESS', transactionId?: string | null, userId?: string | null}): Promise<Response<RevocationHistoryPagingSlicedResult>> {
    const params = {limit: 20, ...queryParams} as AxiosRequestConfig
    const url = '/platform/admin/namespaces/{namespace}/revocation/history'.replace('{namespace}', this.namespace)     
    const resultPromise = this.axiosInstance.get(url, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, RevocationHistoryPagingSlicedResult, 'RevocationHistoryPagingSlicedResult')
  }
