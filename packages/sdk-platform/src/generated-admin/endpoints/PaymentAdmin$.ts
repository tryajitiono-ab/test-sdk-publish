...
export class PaymentAdmin$ {
   ...
   */
  getPaymentOrders( queryParams?: {channel?: 'EXTERNAL' | 'INTERNAL', extTxId?: string | null, limit?: number, offset?: number, status?: 'AUTHORISED' | 'AUTHORISE_FAILED' | 'CHARGEBACK' | 'CHARGEBACK_REVERSED' | 'CHARGED' | 'CHARGE_FAILED' | 'DELETED' | 'INIT' | 'NOTIFICATION_OF_CHARGEBACK' | 'REFUNDED' | 'REFUNDING' | 'REFUND_FAILED' | 'REQUEST_FOR_INFORMATION'}): Promise<Response<PaymentOrderPagingSlicedResult>> {
    const params = {limit: 20, ...queryParams} as AxiosRequestConfig
    const url = '/platform/admin/namespaces/{namespace}/payment/orders'.replace('{namespace}', this.namespace)     
    const resultPromise = this.axiosInstance.get(url, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, PaymentOrderPagingSlicedResult, 'PaymentOrderPagingSlicedResult')
  }
   ...
   */
  getPaymentNotifications( queryParams?: {endDate?: string | null, externalId?: string | null, limit?: number, notificationSource?: 'ADYEN' | 'ALIPAY' | 'CHECKOUT' | 'NEONPAY' | 'PAYPAL' | 'STRIPE' | 'WALLET' | 'WXPAY' | 'XSOLLA', notificationType?: string | null, offset?: number, paymentOrderNo?: string | null, startDate?: string | null, status?: 'ERROR' | 'IGNORED' | 'PROCESSED' | 'WARN'}): Promise<Response<PaymentNotificationPagingSlicedResult>> {
    const params = {limit: 20, ...queryParams} as AxiosRequestConfig
    const url = '/platform/admin/namespaces/{namespace}/payment/notifications'.replace('{namespace}', this.namespace)     
    const resultPromise = this.axiosInstance.get(url, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, PaymentNotificationPagingSlicedResult, 'PaymentNotificationPagingSlicedResult')
  }
  ...
  getStatusPayment_ByPaymentOrderNo(paymentOrderNo:string): Promise<Response<PaymentOrderChargeStatus>> {
    ...
    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, PaymentOrderChargeStatus, 'PaymentOrderChargeStatus')
  }
  ...
  updateSimulateNotificationPayment_ByPaymentOrderNo(paymentOrderNo:string, data: PaymentOrderNotifySimulation): Promise<Response<NotificationProcessResult>> {
    ...
    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, NotificationProcessResult, 'NotificationProcessResult')
  }
