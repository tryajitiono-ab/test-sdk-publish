...
export class KeyGroupAdmin$ {
   ...
   */
  getKeygroups( queryParams?: {limit?: number, name?: string | null, offset?: number, tag?: string | null}): Promise<Response<KeyGroupPagingSlicedResult>> {
    const params = {limit: 20, ...queryParams} as AxiosRequestConfig
    const url = '/platform/admin/namespaces/{namespace}/keygroups'.replace('{namespace}', this.namespace)     
    const resultPromise = this.axiosInstance.get(url, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, KeyGroupPagingSlicedResult, 'KeyGroupPagingSlicedResult')
  }
   ...
   */
  getKeys_ByKeyGroupId(keyGroupId:string,  queryParams?: {limit?: number, offset?: number, status?: 'ACQUIRED' | 'ACTIVE'}): Promise<Response<KeyPagingSliceResult>> {
    const params = {limit: 20, ...queryParams} as AxiosRequestConfig
