...
export class TradeActionAdmin$ {
  ...
  createTradeCommit(data: TradeChainedActionCommitRequest): Promise<Response<TradeChainActionHistoryInfo>> {
    ...
    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, TradeChainActionHistoryInfo, 'TradeChainActionHistoryInfo')
  }
   ...
   */
  getTradeHistoryByCriteria( queryParams?: {limit?: number, offset?: number, status?: 'FAILED' | 'INIT' | 'SUCCESS', type?: string | null, userId?: string | null}): Promise<Response<TradeActionPagingSlicedResult>> {
    const params = {limit: 20, ...queryParams} as AxiosRequestConfig
    const url = '/platform/admin/namespaces/{namespace}/trade/history/byCriteria'.replace('{namespace}', this.namespace)     
    const resultPromise = this.axiosInstance.get(url, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, TradeActionPagingSlicedResult, 'TradeActionPagingSlicedResult')
  }
