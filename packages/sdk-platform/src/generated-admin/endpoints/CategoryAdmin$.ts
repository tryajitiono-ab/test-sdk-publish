...
export class CategoryAdmin$ {
  ...
  getCategoriesBasic( queryParams?: {storeId?: string | null}): Promise<Response<BasicCategoryInfoArray>> {
    ...
    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, BasicCategoryInfoArray, 'BasicCategoryInfoArray')
  }
   ...
   */
  updateCategory_ByCategoryPath(categoryPath:string, data: CategoryUpdate, queryParams: {storeId: string | null}): Promise<Response<FullCategoryInfo>> {
    const params = { ...queryParams} as AxiosRequestConfig
    const url = '/platform/admin/namespaces/{namespace}/categories/{categoryPath}'.replace('{namespace}', this.namespace).replace('{categoryPath}', categoryPath)     
