...
export class Dlc$ {
   ...
   */
  getUsersMeDlcContent( queryParams: {type: 'EPICGAMES' | 'OCULUS' | 'PSN' | 'STEAM' | 'XBOX', includeAllNamespaces?: boolean | null}): Promise<Response<SimpleUserDlcRewardContentsResponse>> {
    const params = { ...queryParams} as AxiosRequestConfig
    const url = '/platform/public/users/me/dlc/content'     
    const resultPromise = this.axiosInstance.get(url, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, SimpleUserDlcRewardContentsResponse, 'SimpleUserDlcRewardContentsResponse')
  }
  
  /**
   * Get dlc reward simple map, only return the sku of durable item reward. 
   */
  getDlcRewardsDurableMap( queryParams: {dlcType: 'EPICGAMES' | 'OCULUS' | 'PSN' | 'STEAM' | 'XBOX'}): Promise<Response<DlcConfigRewardShortInfo>> {
    const params = { ...queryParams} as AxiosRequestConfig
    const url = '/platform/public/namespaces/{namespace}/dlc/rewards/durable/map'.replace('{namespace}', this.namespace)     
    const resultPromise = this.axiosInstance.get(url, {params})

    return Validate.validateOrReturnResponse(this.useSchemaValidation, () => resultPromise, DlcConfigRewardShortInfo, 'DlcConfigRewardShortInfo')
  }
